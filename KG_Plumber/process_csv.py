            ...
            with ThreadPoolExecutor(max_workers=max(1, MAX_WORKERS)) as ex:
                ...
                for fut in as_completed(futs):
                    ...
                    if processed % max(1 , LOG_EVERY) == 0:
                        ...
                        print(f"[progress] {processed}/{total} ({pct:0.1f}%) | avg {avg:0.2f}s/row | elapsed {elapsed:0.1f}s | ETA {eta_sec:0.1f}s | errors {errors}")

                    if SLEEP_SECONDS:
                        time.sleep(SLEEP_SECONDS)

    total_elapsed = time.time() - start_time
    print(f"Processed {processed} rows in {total_elapsed:0.1f}s; errors={errors}; results saved to {OUTPUT_JSONL}")
